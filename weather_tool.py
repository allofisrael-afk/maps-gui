...
def count_points_in_radius(radius_km=20, step_km=5):
    """
...
# בדיקת כמות הנקודות ברדיוס של 20 ק"מ עם פסיעות של 5 ק"מ — רק בהרצה ישירה ולא בייבוא
if __name__ == "__main__":  # מונע הדפסה אוטומטית בעת ייבוא המודול
    print("Number of points:", count_points_in_radius(radius_km=20, step_km=5))

# פונקציה ליצירת רשימת קואורדינטות סביב נקודה מרכזית
def generate_coordinates(lat, lon, radius_km=20, step_km=5):
    """
...
def save_weather_data(weather_data, filename="weather_data.csv"):
    """
    שומרת את כל נתוני מזג האוויר לקובץ CSV.
    :param weather_data: רשימת נתוני מזג האוויר
    :param filename: שם הקובץ לשמירה
    """
    # הפקת כל המפתחות האפשריים מכל הרשומות
    fieldnames = set()
    for data in weather_data:
        fieldnames.update(data.keys())
    fieldnames = list(fieldnames)  # המרת הסט לרשימה לשימוש ב-CSV

    # כתיבת הנתונים לקובץ ה-CSV
...
def create_heatmap_from_csv(csv_file, output_html="heatmap.html"):
    ...
    heatmap_map = folium.Map(location=[center_lat, center_lon], zoom_start=10)  # יצירת המפה

    # הוספת שכבת HeatMap למפה
    HeatMap(
        heat_data,
        gradient={
            0.0: "blue",   # טמפרטורות נמוכות — ערך 0.0 (לא 0.5 כדי למנוע דריסת המפתח)
...
def main():
    ...
    coordinates = generate_coordinates(center_lat, center_lon, radius_km=10, step_km=5)  # יצירת קואורדינטות
