...
# הפרויקט יושב על כונן D:, ואילו pub cache / gradle home ברירת המחדל יושבים על C:.
# מקמפל ה-Kotlin המשולב יש באג ידוע ב-Windows: הוא מנסה לחשב נתיב יחסי בין קבצים
# שנמצאים על כוננים שונים (relativeTo), וזה נכשל עם "different roots". פותרים
# בכך שממקמים את שני המטמונים על אותו כונן כמו הפרויקט (D:).
$DEV_CACHE = (Split-Path -Qualifier $PSScriptRoot) + "\dev-cache"
$env:PUB_CACHE         = "$DEV_CACHE\pub_cache"
$env:GRADLE_USER_HOME  = "$DEV_CACHE\gradle_home"
New-Item -ItemType Directory -Force $env:PUB_CACHE, $env:GRADLE_USER_HOME | Out-Null
...
# הורדה עמידה לחיבור איטי/לא יציב: curl.exe עם --continue-at - (חידוש הורדה חלקית)
# ו---retry לניסיונות חוזרים אוטומטיים על ניתוקים, במקום Invoke-WebRequest שמתחיל מאפס בכל כשל
function Download-Resumable($url, $outFile) {
    $maxAttempts = 8
    $prevEAP = $ErrorActionPreference
    $ErrorActionPreference = 'Continue'  # curl כותב את התקדמותו ל-stderr; עם 'Stop' זה היה הופך לשגיאה קטלנית
    for ($i = 1; $i -le $maxAttempts; $i++) {
        Log "curl attempt $i/$maxAttempts -> $outFile"
        & curl.exe -s -S -L --fail --retry 3 --retry-delay 5 -C - -o $outFile $url 2>$null
        if ($LASTEXITCODE -eq 0) { $ErrorActionPreference = $prevEAP; return }
        Log "curl exited with code $LASTEXITCODE, retrying..."
        Start-Sleep -Seconds 5
    }
    $ErrorActionPreference = $prevEAP
    Err "Failed to download $url after $maxAttempts attempts"
}
...
if (-not (Test-Path "$FLUTTER_DIR\bin\flutter.bat")) {
    ...
    Download-Resumable $FLUTTER_URL $zipFlutter
...
if (-not (Test-Path "$CMDTOOLS_DIR\bin\sdkmanager.bat")) {
    ...
    Download-Resumable $CMDTOOLS_URL $zipTools
...
# מכאן ואילך אנחנו קוראים בעיקר לתהליכים חיצוניים (sdkmanager, flutter) שכותבים
# פלט שגרתי (לא שגיאות אמיתיות) ל-stderr — עם 'Stop' זה הופך לשגיאה קטלנית מיד.
# עוברים ל-'Continue' ובודקים $LASTEXITCODE במפורש אחרי כל שלב קריטי במקום זאת.
$ErrorActionPreference = 'Continue'

# --- Step 4: קבל רישיונות ---
# הזרמת "y" בפייפ ל-sdkmanager.bat (עטיפת cmd סביב java) לא אמינה — קלט ה-stdin
# מגיע לפני שהתהליך מוכן לקרוא אותו, וההסכמה "לא נשמרת" בפועל בכל הרצה.
# הפתרון היציב: כותבים ישירות את קבצי ה-hash של הרישיונות הידועים תחת licenses\,
# בדיוק כמו ש-sdkmanager --licenses עצמו היה עושה לאחר אישור אינטראקטיבי.
Log "Accepting Android licenses..."
$licensesDir = "$ANDROID_DIR\licenses"
New-Item -ItemType Directory -Force $licensesDir | Out-Null
Set-Content -Path "$licensesDir\android-sdk-license" -Value "24333f8a63b6825ea9c5514f83c2829b004d1fee" -Encoding ASCII -NoNewline
Add-Content -Path "$licensesDir\android-sdk-license" -Value "`nd56f5187479451eabf01fb78af6dfcb131a6481e" -Encoding ASCII
Set-Content -Path "$licensesDir\android-sdk-preview-license" -Value "84831b9409646a918e30573bab4c9c91346d8abd" -Encoding ASCII -NoNewline
Ok "Licenses accepted"

# --- Step 5: SDK packages ---
if (-not (Test-Path "$ANDROID_DIR\build-tools\35.0.0")) {
    Log "Installing Android SDK packages..."
    & "$CMDTOOLS_DIR\bin\sdkmanager.bat" --sdk_root=$ANDROID_DIR `
        "platform-tools" "build-tools;35.0.0" "platforms;android-35" 2>$null
    if (-not (Test-Path "$ANDROID_DIR\build-tools\35.0.0")) { Err "Android SDK package install failed - build-tools;35.0.0 missing" }
...
# הערה: $LASTEXITCODE אחרי flutter.bat pub get לא אמין תמיד על Windows (נצפה קוד שגיאה
# גם כשה-resolve בפועל הצליח לגמרי) — הקריטריון האמיתי להצלחה הוא קובץ pubspec.lock עדכני.
Set-Location $PROJECT_DIR
Log "Running flutter pub get..."
Remove-Item "$PROJECT_DIR\pubspec.lock" -ErrorAction SilentlyContinue
& "$FLUTTER_DIR\bin\flutter.bat" pub get 2>$null
if (-not (Test-Path "$PROJECT_DIR\pubspec.lock")) { Err "flutter pub get failed - pubspec.lock not created" }
Ok "pub get succeeded"

# --- Step 9: Build APK ---
# הערה: כמו ב-pub get, $LASTEXITCODE של flutter.bat לא תמיד אמין על Windows —
# לא בודקים אותו כתנאי עצירה; הקריטריון האמיתי הוא קובץ ה-APK עצמו, טרי מהבנייה הזו.
$buildStart = Get-Date
Log "Building release APK..."
& "$FLUTTER_DIR\bin\flutter.bat" build apk --release

$APK = "$PROJECT_DIR\build\app\outputs\flutter-apk\app-release.apk"
if ((Test-Path $APK) -and (Get-Item $APK).LastWriteTime -ge $buildStart) {
