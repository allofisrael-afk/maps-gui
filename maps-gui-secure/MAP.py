...
<head>
    ...
    <style>
        ...
        /* tooltip טמפרטורה בנקודות דגימה */
        .temp-tooltip {{
        ...
        .temp-tooltip::before {{ display: none; }}
...
<body>
    ...
    <script>
        ...
        var tempDotLayer    = null;   // שכבת נקודות דגימה — חלופה לשכבת החום
        var tempGridLayer   = null;   // שכבת גריד תאים — כיסוי אחיד ללא רווחים
        var _tempGridData   = [];     // נתוני גריד גולמיים — משותפים לשתי שכבות
        var tempSelectStart = null;
        var tempPreviewRect = null;
        var tempSelecting   = false;
        var serversRunning  = false;
        ...
        var elevSelectStart  = null;
        var elevPreviewRect  = null;
        var elevSelecting    = false;
        var _elevGridData    = [];    // נתוני גריד גבהים — משותפים לשלוש תצוגות
        var elevDotLayer     = null;  // שכבת נקודות גבהים
        var elevCellLayer    = null;  // שכבת גריד תאי גובה

        function startElevationSelection() {{
            if (elevationLayer) {{ map.removeLayer(elevationLayer); elevationLayer = null; }}
            if (elevDotLayer)   {{ map.removeLayer(elevDotLayer);   elevDotLayer   = null; }}
            if (elevCellLayer)  {{ map.removeLayer(elevCellLayer);  elevCellLayer  = null; }}
            _elevGridData  = [];
        ...
        function _finishElevSelection(e) {{
            ...
            _loadElevationGrid(bounds);
        }}

        function _loadElevationGrid(bounds) {{
            var sw = bounds.getSouthWest();
            var ne = bounds.getNorthEast();

            fetch('http://localhost:5002/elevation_grid', {{
                method:  'POST',
            ...
            .then(function(r) {{
                if (r.status === 429) throw new Error('חריגת מגבלת API — נסה שנית בעוד דקה');
                if (!r.ok) throw new Error('שגיאת שרת: ' + r.status);
                return r.json();
            }})
            .then(function(data) {{
                if (data.error) throw new Error(data.error);
                if (!data.length) throw new Error('אין נתוני גובה');
                _elevGridData  = data;
                showElevAsHeatmap();
                elevationActive = true;
                document.title  = '';
                document.title  = '__elevation_ok__';
            }})
            .catch(function(err) {{
                console.error('שגיאת שכבת גבהים:', err);
                elevationActive = false;
                document.title  = '';
                document.title  = '__elevation_error__:' + (err.message || 'שגיאה לא ידועה');
            }});
        }}

        function _calcElevRadius() {{
            var zoom     = map.getZoom();
            var midLat   = map.getCenter().lat;
            var pixPerKm = 256 * Math.pow(2, zoom) / 360 / 111 * Math.cos(midLat * Math.PI / 180);
            var stepKm   = 0.5;
            if (_elevGridData.length > 1) {{
                var sortedLats = _elevGridData.map(function(p) {{ return p.lat; }})
                                              .sort(function(a, b) {{ return a - b; }});
                for (var i = 1; i < sortedLats.length; i++) {{
                    var d = (sortedLats[i] - sortedLats[i - 1]) * 111.0;
                    if (d > 0.05) {{ stepKm = d; break; }}
                }}
            }}
            return Math.max(6, Math.ceil(pixPerKm * stepKm * 2.5));
        }}

        // מחזיר [r,g,b] לפי ערך נורמלי 0-1 — משמש לרינדור canvas
        function _colorRgb(norm) {{
            var stops = [
                [0.00, [0,   0,   204]],
                [0.25, [0,   170, 255]],
                [0.50, [0,   255, 204]],
            ...
            for (var i = 0; i < stops.length - 1; i++) {{
                if (norm <= stops[i + 1][0]) {{ lo = stops[i]; hi = stops[i + 1]; break; }}
            }}
            var t = (norm - lo[0]) / ((hi[0] - lo[0]) || 1);
            return [
        ...
        // שכבת חום לגבהים — canvas עם אינטרפולציה בילינארית
        // כל פיקסל מחושב מ-4 נקודות הסביבה → גרדיאנט חלק ומדויק לפי ערכי הגובה האמיתיים
        function showElevAsHeatmap() {{
            if (elevDotLayer)  {{ map.removeLayer(elevDotLayer);  elevDotLayer  = null; }}
            if (elevCellLayer) {{ map.removeLayer(elevCellLayer); elevCellLayer = null; }}
            if (elevationLayer) {{ map.removeLayer(elevationLayer); elevationLayer = null; }}
            if (!_elevGridData.length) return;

            var elevs  = _elevGridData.map(function(p) {{ return p.elevation; }});
            var minE   = Math.min.apply(null, elevs);
            var maxE   = Math.max.apply(null, elevs);
            var range  = maxE - minE || 1;

            var allLats = _elevGridData.map(function(p) {{ return p.lat; }});
            var allLons = _elevGridData.map(function(p) {{ return p.lng; }});
            ...
            var uLats = allLats.filter(function(v,i,a){{ return a.indexOf(v)===i; }})
                               .sort(function(a,b){{return a-b;}});
            var uLons = allLons.filter(function(v,i,a){{ return a.indexOf(v)===i; }})
                               .sort(function(a,b){{return a-b;}});

            var eMap = {{}};
            _elevGridData.forEach(function(p) {{
                eMap[p.lat.toFixed(6)+'_'+p.lng.toFixed(6)] = p.elevation;
            }});

            function bilinear(lat, lon) {{
                lat = Math.max(minLat, Math.min(maxLat, lat));
                lon = Math.max(minLon, Math.min(maxLon, lon));
                var li = 0, hi = uLats.length - 1;
                while (li < hi - 1) {{ var m = (li+hi)>>1; if (uLats[m] <= lat) li=m; else hi=m; }}
                var lj = 0, hj = uLons.length - 1;
                while (lj < hj - 1) {{ var m = (lj+hj)>>1; if (uLons[m] <= lon) lj=m; else hj=m; }}
                var y0=uLats[li], y1=uLats[hi], x0=uLons[lj], x1=uLons[hj];
                var ty = y0===y1 ? 0 : (lat-y0)/(y1-y0);
                var tx = x0===x1 ? 0 : (lon-x0)/(x1-x0);
                var e00 = eMap[y0.toFixed(6)+'_'+x0.toFixed(6)] || minE;
                var e01 = eMap[y0.toFixed(6)+'_'+x1.toFixed(6)] || minE;
                var e10 = eMap[y1.toFixed(6)+'_'+x0.toFixed(6)] || minE;
                var e11 = eMap[y1.toFixed(6)+'_'+x1.toFixed(6)] || minE;
                return e00*(1-ty)*(1-tx) + e01*(1-ty)*tx +
                       e10*ty*(1-tx)     + e11*ty*tx;
            }}
            ...
            for (var y = 0; y < H; y++) {{
                for (var x = 0; x < W; x++) {{
                    var lat  = maxLat - (y / (H-1)) * (maxLat - minLat);
                    var lon  = minLon + (x / (W-1)) * (maxLon - minLon);
                    var elev = bilinear(lat, lon);
                    var norm = (elev - minE) / range;
                    var rgb  = _colorRgb(norm);
                    var i4   = (y * W + x) * 4;
                    img.data[i4]   = rgb[0];
                    img.data[i4+1] = rgb[1];
                    img.data[i4+2] = rgb[2];
        ...
        function showElevAsGrid() {{
            if (elevationLayer) {{ map.removeLayer(elevationLayer); elevationLayer = null; }}
            if (elevDotLayer)   {{ map.removeLayer(elevDotLayer);   elevDotLayer   = null; }}
            _buildElevCellLayer();
        }}

        function showElevAsDots() {{
            if (elevationLayer) {{ map.removeLayer(elevationLayer); elevationLayer = null; }}
            if (elevCellLayer)  {{ map.removeLayer(elevCellLayer);  elevCellLayer  = null; }}
            _buildElevDotLayer();
        }}

        function _buildElevDotLayer() {{
            if (elevDotLayer) {{ map.removeLayer(elevDotLayer); elevDotLayer = null; }}
            if (!_elevGridData.length) return;
            var elevs = _elevGridData.map(function(p) {{ return p.elevation; }});
            var minE  = Math.min.apply(null, elevs);
            var maxE  = Math.max.apply(null, elevs);
            var range = maxE - minE || 1;
            var markers = _elevGridData.map(function(p) {{
                var norm  = (p.elevation - minE) / range;
                var color = _tempColor(norm);
                return L.circleMarker([p.lat, p.lng], {{
                    radius: 3, color: color, fillColor: color,
                    fillOpacity: 0.85, weight: 1, opacity: 0.9
                }}).bindTooltip(Math.round(p.elevation) + ' מטר', {{
                    permanent: false, direction: 'top', className: 'temp-tooltip'
                }});
            }});
            elevDotLayer = L.layerGroup(markers).addTo(map);
        }}

        function _buildElevCellLayer() {{
            if (elevCellLayer) {{ map.removeLayer(elevCellLayer); elevCellLayer = null; }}
            if (!_elevGridData.length) return;
            var elevs = _elevGridData.map(function(p) {{ return p.elevation; }});
            var minE  = Math.min.apply(null, elevs);
            var maxE  = Math.max.apply(null, elevs);
            var range = maxE - minE || 1;
            var sortedLats = _elevGridData.map(function(p) {{ return p.lat; }}).sort(function(a,b){{return a-b;}});
            var sortedLons = _elevGridData.map(function(p) {{ return p.lng; }}).sort(function(a,b){{return a-b;}});
            var halfLat = 0.025, halfLon = 0.03;
            for (var i = 1; i < sortedLats.length; i++) {{
                var d = sortedLats[i] - sortedLats[i-1];
                if (d > 0.0001) {{ halfLat = d / 2 * 1.02; break; }}
            }}
            for (var i = 1; i < sortedLons.length; i++) {{
                var d = sortedLons[i] - sortedLons[i-1];
                if (d > 0.0001) {{ halfLon = d / 2 * 1.02; break; }}
            }}
            var rects = _elevGridData.map(function(p) {{
                var norm  = (p.elevation - minE) / range;
                var color = _tempColor(norm);
                return L.rectangle(
                    [[p.lat - halfLat, p.lng - halfLon], [p.lat + halfLat, p.lng + halfLon]],
                    {{ weight: 0, fillColor: color, fillOpacity: 0.72 }}
                ).bindTooltip(Math.round(p.elevation) + ' מטר', {{
                    permanent: false, direction: 'top', className: 'temp-tooltip'
                }});
            }});
            elevCellLayer = L.layerGroup(rects).addTo(map);
        }}
        ...
        function _finishTempSelection(e) {{
            ...
            if (Math.abs(ne.lat - sw.lat) < 0.001 || Math.abs(ne.lng - sw.lng) < 0.001) {{
                document.title = '__heatmap_cancel__';
                return;
            }}
        ...
        function _loadTempHeatmap(bounds) {{
            ...
            .then(function(data) {{
                if (!data.length) throw new Error('אין נתוני טמפרטורה');

                _tempGridData = data;
                showTempAsHeatmap();

                document.title = '';
                document.title = '__heatmap_ok__';
            }})
            .catch(function(err) {{
                console.error('שגיאת מפת חום:', err);
                document.title = '';
                document.title = '__heatmap_error__';
            }});
        }}

        function clearTempHeatmap() {{
            if (tempHeatLayer) {{ map.removeLayer(tempHeatLayer); tempHeatLayer = null; }}
            if (tempDotLayer)  {{ map.removeLayer(tempDotLayer);  tempDotLayer  = null; }}
        ...
        function showTempAsGrid() {{
            if (tempHeatLayer) {{ map.removeLayer(tempHeatLayer); tempHeatLayer = null; }}
            if (tempDotLayer)  {{ map.removeLayer(tempDotLayer);  tempDotLayer  = null; }}
            _buildGridLayer();
        }}

        // שכבת חום לטמפרטורות — canvas עם אינטרפולציה בילינארית
        // כל פיקסל מחושב מ-4 נקודות הסביבה → גרדיאנט חלק ומדויק לפי ערכי הטמפרטורה האמיתיים
        function showTempAsHeatmap() {{
            if (tempGridLayer) {{ map.removeLayer(tempGridLayer); tempGridLayer = null; }}
            if (tempDotLayer)  {{ map.removeLayer(tempDotLayer);  tempDotLayer  = null; }}
            if (tempHeatLayer) {{ map.removeLayer(tempHeatLayer); tempHeatLayer = null; }}
            if (!_tempGridData.length) return;

            var temps  = _tempGridData.map(function(p) {{ return p.temperature; }});
            var minT   = Math.min.apply(null, temps);
            var maxT   = Math.max.apply(null, temps);
            var range  = maxT - minT || 1;

            var allLats = _tempGridData.map(function(p) {{ return p.lat; }});
            var allLons = _tempGridData.map(function(p) {{ return p.lng; }});
            var minLat  = Math.min.apply(null, allLats);
            var maxLat  = Math.max.apply(null, allLats);
            var minLon  = Math.min.apply(null, allLons);
            var maxLon  = Math.max.apply(null, allLons);
            ...
            var tMap = {{}};
            _tempGridData.forEach(function(p) {{
                tMap[p.lat.toFixed(6)+'_'+p.lng.toFixed(6)] = p.temperature;
            }});

            var bilinearTemp = function(lat, lon) {{
                lat = Math.max(minLat, Math.min(maxLat, lat));
                lon = Math.max(minLon, Math.min(maxLon, lon));
                var li = 0, hi = uLats.length - 1;
                while (li < hi - 1) {{ var m = (li+hi)>>1; if (uLats[m] <= lat) li=m; else hi=m; }}
                ...
                var ty = y0===y1 ? 0 : (lat-y0)/(y1-y0);
                var tx = x0===x1 ? 0 : (lon-x0)/(x1-x0);
                var t00 = tMap[y0.toFixed(6)+'_'+x0.toFixed(6)]; if(t00===undefined) t00=minT;
                var t01 = tMap[y0.toFixed(6)+'_'+x1.toFixed(6)]; if(t01===undefined) t01=minT;
                var t10 = tMap[y1.toFixed(6)+'_'+x0.toFixed(6)]; if(t10===undefined) t10=minT;
                var t11 = tMap[y1.toFixed(6)+'_'+x1.toFixed(6)]; if(t11===undefined) t11=minT;
                return t00*(1-ty)*(1-tx) + t01*(1-ty)*tx +
                       t10*ty*(1-tx)     + t11*ty*tx;
            }};
            ...
            for (var y = 0; y < H; y++) {{
                for (var x = 0; x < W; x++) {{
                    var lat  = maxLat - (y / (H-1)) * (maxLat - minLat);
                    var lon  = minLon + (x / (W-1)) * (maxLon - minLon);
                    var temp = bilinearTemp(lat, lon);
                    var norm = (temp - minT) / range;
                    var rgb  = _colorRgb(norm);
                    var i4   = (y * W + x) * 4;
                    img.data[i4]   = rgb[0];
                    img.data[i4+1] = rgb[1];
        ...
        function showTempAsDots() {{
            if (tempHeatLayer) {{ map.removeLayer(tempHeatLayer); tempHeatLayer = null; }}
            if (tempGridLayer) {{ map.removeLayer(tempGridLayer); tempGridLayer = null; }}
            _buildDotLayer();
        }}

        function _tempColor(norm) {{
            var stops = [
                [0.00, [0,   0,   204]],
                [0.25, [0,   170, 255]],
                [0.50, [0,   255, 204]],
        ...
        function _buildDotLayer() {{
            ...
            var markers = _tempGridData.map(function(p) {{
                var norm  = (p.temperature - minT) / range;
                var color = _tempColor(norm);
                return L.circleMarker([p.lat, p.lng], {{
                    radius:      3,
                    color:       color,
                    ...
                    weight:      1,
                    opacity:     0.9
                }}).bindTooltip(p.temperature.toFixed(1) + '°C', {{
                    permanent:  false,
                    direction:  'top',
            ...
            tempDotLayer = L.layerGroup(markers).addTo(map);
        }}

        function _buildGridLayer() {{
            if (tempGridLayer) {{ map.removeLayer(tempGridLayer); tempGridLayer = null; }}
            if (!_tempGridData.length) return;
            ...
            // חישוב גודל תא: מרחק מינימלי בין נקודות סמוכות + מרווח חפיפה קטן
            var sortedLats = _tempGridData.map(function(p) {{ return p.lat; }}).sort(function(a,b){{return a-b;}});
            var sortedLons = _tempGridData.map(function(p) {{ return p.lng; }}).sort(function(a,b){{return a-b;}});
            var halfLat = 0.025, halfLon = 0.03;
            for (var i = 1; i < sortedLats.length; i++) {{
                var d = sortedLats[i] - sortedLats[i-1];
                if (d > 0.0001) {{ halfLat = d / 2 * 1.02; break; }}
            ...
            var rects = _tempGridData.map(function(p) {{
                var norm  = (p.temperature - minT) / range;
                var color = _tempColor(norm);
                return L.rectangle(
            ...
            tempGridLayer = L.layerGroup(rects).addTo(map);
        }}
        ...
        function _calcTempRadius() {{
            ...
            // הסקת גודל הצעד מהנתונים — עובד עם כל רזולוציה אדפטיבית
            var stepKm = 0.5;
            if (_tempGridData.length > 1) {{
                var sortedLats = _tempGridData.map(function(p) {{ return p.lat; }})
                                              .sort(function(a, b) {{ return a - b; }});
                for (var i = 1; i < sortedLats.length; i++) {{
                    var d = (sortedLats[i] - sortedLats[i - 1]) * 111.0;
                    if (d > 0.05) {{ stepKm = d; break; }}
        ...
        function _calcTempBlur(r) {{
            return Math.max(4, Math.round(r * 0.8));
        }}
        ...
        // לחיצה על המפה — כאשר picker פעיל, מוסיף נקודת חום
        ...
        map.on('zoomend', function() {{
            // tempHeatLayer is now an L.imageOverlay — no zoom-dependent recalculation needed
        }});
        ...
        function fetchWeather(lat, lon) {{
            // שליפת נתוני מזג אוויר לפי קואורדינטות
            ...
            fetch(url)
                ...
                .then(data => {{
                    // ולידציה — בדיקה שהתשובה כוללת את השדות הנדרשים
                    if (!data.weather || !data.temperature) {{
                        throw new Error("הנתונים שהתקבלו אינם כוללים מזג אוויר או טמפרטורה");
                    }}
                    displayWeatherOnMap(lat, lon, data);  // הצגת הנתונים על המפה
                }})
                .catch(error => {{
        ...
        function displayWeatherOnMap(lat, lon, data) {{
            try {{
                ...
                // תוכן Popup — נתוני מזג אוויר בעברית
                const content = `
