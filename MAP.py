    ...
    <script>
        ...
        map.on('click', function(e) {{
            ...
            if (heatmapPickerActive) {{
                ...
                clearRadialLosResult();  // ניקוי תוצאה/תצוגה מקדימה קודמת — משקיף חדש בכל לחיצה
                radialLosObs = e.latlng;
                radialLosObsMk = L.marker(e.latlng, {{icon: _radialLosMarkerIcon(), draggable: true}}).addTo(map);
        ...
        var LosControl = L.Control.extend({{
            ...
            onAdd: function() {{
                ...
                L.DomEvent.on(btn, 'click', function(e) {{
                    ...
                    if (!_mapReady) return;  // הגנה נוספת מעבר ל-pointer-events — המפה עדיין לא מוכנה
                    toggleLos();
                }});
                ...
                losFieldsWrap.style.cssText = 'display:none;align-items:center;gap:5px;';
                _losFieldsWrapEl = losFieldsWrap;

                function _mkHeightField(labelText, defaultVal, tip) {{
                    var wrap = L.DomUtil.create('label', '', losFieldsWrap);
